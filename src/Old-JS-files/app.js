...
$(document).ready(function () {
  ...
  $('#addPersonDiv').click(function (e) {
    ...
    if (x < max_fields) {
      var addPerson = $(
        "<div class='input-field col s12 m12 center-align newPersonDiv'>"
      );
      var newInput = $(
        "<input placeholder='Phone Number or Email' type='text' class='validate addPerson'>"
      );
      // if you change the <a> tag to a <button> tag it works, but refreshes the page everytime you press it :(
      var deleteBtn = $("<a class='deletePerson'>").html(
  ...
  $('#sendInvite').on('click', function (event) {
    ...
    $('.addPerson').each(function (_i, elem) {
      console.log(elem);
      allEmails.push($(elem).val());
    });
    $('#inputPlace').each(function (_i, elem) {
      console.log(elem);
  ...
  var y = document.getElementById('error-handling');
